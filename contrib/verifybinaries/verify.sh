...
###   tcccoincore.org and tcccoin.org and compares them.
...
WORKINGDIR="/tmp/tcccoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://tcccoincore.org"
HOST2="https://tcccoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="tcccoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'tcccoin-',
...
if [ $? -ne 0 ]; then
   echo "tcccoin.org failed to provide signature file, but tcccoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "tcccoin.org and tcccoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the tcccoin Core binary release signing key installed?"
   fi
