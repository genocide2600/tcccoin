#!/bin/bash
# Copyright (c) 2013 The tcccoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete tcccoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no tcccoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old tcccoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected tcccoin 0.7 datadir."
    ;;
  3)
    echo "Detected tcccoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected tcccoin 0.8 datadir."
    ;;
esac
