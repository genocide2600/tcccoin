...
The RPM spec file provided here is for tcccoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://tcccoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[tcccoin git repository](https://github.com/tcccoin/tcccoin) but are not part of
...
Currently only `Source10` is used, for the example `tcccoin.conf` file.
...
in the tcccoin git repository. At present only an SVG version of the tcccoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the tcccoin
...
needed when 0.12.1 is released, a proper fix is already in the tcccoin git
master branch.
...
idea to build tcccoin Core against a static version of that library compiled
...
The `tcccoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `tcccoin-server` package. At this point, most RPM based
...
The `tcccoin-server` package should have SELinux support. How to properly do
...
tcccoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/tcccoin/). He has been
packaging tcccoin for Fedora at least since 2012.
...
4. I renamed the `tcccoin` package that contains the Qt GUI to `tcccoin-core` as
that appears to be how the general population refers to it, in contrast to
`tcccoin-xt` or `tcccoin-classic`. I wanted to make sure the general population
