 ...
 * between tcccoind, and tcccoin-qt and non-server tools.
*/
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), tcccoin_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/tcccoin/tcccoin>";
    const std::string URL_WEBSITE = "<https://tcccoincore.org>";
...
/** Sanity checks
 *  Ensure that tcccoin is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single tcccoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", tcccoin_CONF_FILENAME)).string());
